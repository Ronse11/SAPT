...
function triggerPrint() {
  ...
  // Determine the max rows and columns considering colspan and rowspan
  ...
  // Process each row and ensure colspan and rowspan are included in the printed table
  rows.slice(0, maxRowIndex + 1).forEach((row, rowIndex) => {
    ...
    cells.forEach((cell) => {
      ...
      // Add cell with colspan, rowspan, inline styles, and yellow background
      rowContent += `
        <td 
        const fontColor = style.color || "inherit";
          style="border: 1px solid black; padding: 4px; ${inlineStyles} color: ${fontColor}; background-color: ${backgroundColor}" 
          rowspan="${rowspan}" 
          colspan="${colspan}">
          ${content || "&nbsp;"}
        </td>`;
    ...
    while (colIndex <= maxColIndex) {
      rowContent += `<td style="border: 1px solid black; padding: 4px;">&nbsp;</td>`;
      colIndex++;
    }
  ...
  // Create a unique print container with a unique ID
  const printContainer = document.createElement("div");
  printContainer.id = "unique-print-container"; // Unique ID for the print container
  printContainer.style.display = "none";
  printContainer.innerHTML = `
    <style>
      @media print {
        @page { margin: 1cm; }
        body > *:not(#unique-print-container) { display: none !important; }
        #unique-print-container { display: block !important; }
      }
    </style>
    ${printContent}
  `;
  ...
  // Use PrintJS to print the filtered table content
  PrintJS({
    printable: 'unique-print-container', // Use the unique ID
    type: 'html',
    style: `
      table {
        width: 100%;
    ...
    `,
    scanStyles: false // Disable scanning of styles
  });
