    ...
    <section class=" w-full h-screen grid place-content-center">
        <div class=" w-full h-full grid place-items-center">
            <div class=" w-full max-w-[26rem] p-8 shadow-md rounded-md flex flex-col gap-y-4 bg-bgcolor">
                ...
                <form action=" {{route('trackFolder', $folderId->id) }} " method="POST" autocomplete="off">
                    @csrf
                    <div class="flex flex-col gap-y-6">
                        <label for="code_number" class="font-semibold">Enter your Track Code :</label>
                        <input type="text" id="code_number" name="code_number" class=" px-3 py-2 border-2 border-subtText outline-mainText rounded-md @error('error') ring-2 ring-red-500 @enderror" placeholder="Track Code">
                    </div>
                    @error('code_number')
                    ...
                    <div class="flex flex-col gap-y-4">
                        <h1 class="text-sm text-subtText mt-1">example: its-oki-oki</h1>
                    </div>
                    {{-- Full Name of User --}}
                    <div class="flex flex-col gap-y-6">
