...
<body>
    <section
        ...
        {{-- SUCCESS --}}
        <div id="success-changeName"
            class="absolute inset-0 flex items-center justify-center z-50 transition-all duration-500 ease-in-out opacity-0 pointer-events-none transform translate-y-[-50px]">
            <div
                class="pt-6 pb-14 px-10 bg-white rounded-md border border-sgline shadow-lg max-w-sm w-full pointer-events-none">
        ...
        <nav class="sidebar flex flex-col w-[17%] h-full bg-gray-100 absolute top-0 left-0 border-r border-sgline">
            ...
            <div class="flex-1 flex flex-col relative overflow-y-auto">
                ...
                <div class=" flex flex-col class-nav overflow-y-auto overflow-x-hidden hide-scrollbar px-4"
                    id="scrollableDiv">
                    @if ($allRooms->isEmpty())
                        <div></div>
                    @else
                ...
                <div class="pl-4 px-4 border-b border-sgline flex flex-col folder-nav overflow-y-auto overflow-x-hidden hide-scrollbar relative"
                    id="scrollableDiv">
                    @if ($folders->isEmpty())
                        <div></div>
                    @else
        ...
        @auth
            ...
            <section id="hero"
                ...
                <div id="show-bar"
                    class="opacity-0 pointer-events-none z-50 absolute md:hidden top-0 left-0 bottom-0 w-0 transition-all duration-500 ease-in-out">
                    <div
                        class="w-full h-full shadow-xl pr-7 pl-4 py-4 flex flex-col border-l border-sgline gap-6 bg-mainBg">
                        <nav class=" flex flex-col w-full h-full bg-gray-100 absolute top-0 left-0 border-r border-sgline">
                            <div class="flex h-[5.5rem] items-center justify-between pl-8 border-b border-sgline">
                                <div class="flex items-center">
                                    <img class=" w-8 h-8 mb-1 mr-1"
                                        src="{{ Vite::asset('resources/images/saptlogo.svg') }}" alt="Logo">
                                    <h1
                                        class=" text-3xl mt-2 font-normal text-black select-none tracking-widest cp:text-3xl">
                                        APT</h1>
                                </div>
                                <div class="relative group">
                                    <button id="close-bar" class=" -mb-1 text-4xl pt-1 cp:text-3xl"><i
                                            class='bx bx-menu text-mainText hover:text-subtText mr-6'></i></button>
                                    <div
                                        class="absolute top-full -left-[50%] transform -translate-x-1/2 mt-3 hidden group-hover:flex flex-col items-center mb-3">
                                        <div
                                            class="relative w-[8rem] bg-mainText bg-opacity-85 text-white text-sm rounded-lg py-2 px-4">
                                            <h1>Close sidebar</h1>
                                            <div
                                                class="absolute -top-1 left-[82%] transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-b-4 border-transparent border-b-gray-700 bg-opacity-75">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div
                    class="row-start-1 row-span-8 col-start-1 col-span-8 overflow-y-auto overflow-scrolls z-10 bg-bgcolor">
                    <div
                        class="w-full flex flex-col justify-center items-center h-[5.5rem] cp:pr-8 cp:pl-16 p-4 tablet:items-start sticky top-0 left-0 z-40 cp:gap-8 bg-bgcolor border-b border-sgline">
                        {{-- Header --}}
                        <header
                            class=" w-full row-start-1 row-span-1 flex justify-between items-center tablet:shadow-none z-40">
                                ...
                                <div class="show hidden justify-self-start w-full">
                                    <div class="flex w-full items-center justify-between">
                                        <div class="flex-1 flex items-center justify-between">
                                            <div class=" flex items-center">
                                                <img class=" w-8 h-8 mb-3 mr-1"
                                                    src="{{ Vite::asset('resources/images/saptlogo.svg') }}"
                                                    alt="Logo">
                                                <h1
                                                    class=" text-xl font-normal text-black select-none tracking-widest cp:text-3xl">
                                                    APT</h1>
                                            </div>
                                            <div class="relative group">
                                                <button id="menu-assist" class="  -mb-1 text-4xl pt-1 cp:text-3xl"><i
                                                        class='bx bx-menu text-mainText hover:text-subtText'></i></button>
                                                <div
                                                    class="absolute top-full left-1/2 transform -translate-x-1/2 mt-3 hidden group-hover:flex flex-col items-center mb-3">
                                                    <div
                                                        class="relative w-[8rem] bg-mainText bg-opacity-85 text-white text-sm rounded-lg py-2 px-4">
                                                        <h1>Open sidebar</h1>
                                                        <div
                                                            class="absolute -top-1 left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-b-4 border-transparent border-b-gray-700 bg-opacity-75">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            {{-- SEARCH SECTION HERE! --}}
                            <div
                                class=" h-full w-full row-start-1 row-span-1 cp:flex hidden items-center col-start-4 pr-16 z-40">
                                <div class=" cp:flex hidden items-center h-12 w-full rounded-full relative ">
                                    ...
                                    <input type="text" name="search" id="search"
                                        class="search-btn h-full w-full rounded-md text-base px-14 border border-sgline outline-subtText bg-gray-100 hover:bg-sgcolorSub hover:border-sgcolorSub placeholder"
                            ...
                            <div id="class-list" class="content-box flex flex-col items-center h-auto p-4 md:p-16 gap-8">
                                <div class=" w-full p-6 border border-sgline rounded-md">
                                    ...
                                    <div class=" w-full flex items-center gap-2 mb-5">
                                        <div
                                            class=" text-3xl w-9 h-9 grid place-items-center rounded-full cp:text-2xl bg-mainText">
                                            <h1 class=" text-xl text-bgcolor">{{ substr(auth()->user()->username, 0, 1) }}
                                            </h1>
                                        </div>
                                    ...
                                    <h1 class="text-sm font-normal text-mainText mb-5">Change your password and security
                                        options, and access other Google services. <a href=""
                                            class=" underline text-blue-500">Manage</a></h1>
                                    <h1 class="text-sm font-medium text-mainText mb-1">Change name</h1>
                                    <input type="text" name="changeName" id="changeName"
                                        class=" outline-none border-b border-mainText @error('changeName') ring-2 ring-red-500 @enderror"
                                        value="{{ $userName }}">
                                </div>
                                <div class=" w-full p-6 border border-sgline rounded-md">
                                    <h1 class=" text-4xl mb-5">Notifications</h1>
                                    <h1 class="text-2xl font-normal text-mainText mb-1">SMS</h1>
                                    <h1 class="text-sm font-normal text-mainText mb-5">These settings apply to the
                                        notifications you get by sms.
                                </div>
                                <div class=" w-full p-6 border border-sgline rounded-md">
                                    <div>
                                        <h1 class=" text-4xl mb-5">Role</h1>
                                        <h1 class="text-sm font-normal text-mainText mb-10">These settings allow you to
                                            change your role.
                                    </div>
                                    ...
                                    <form method="POST" action="{{ route('user.updateRole') }}">
                                        ...
                                        <div class="text-sm font-medium text-mainText mb-1 w-full">
                                            <select name="role" id="role"
                                                class="w-full cursor-pointer outline-none" onchange="this.form.submit()">
